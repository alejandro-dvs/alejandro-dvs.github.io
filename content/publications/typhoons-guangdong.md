---
title: "The Short-Term Economic Impact of Tropical Cyclones: Satellite Evidence from Guangdong Province"
authors:
  - A. Del Valle
  - R. Elliott
  - E. Strobl
  - M. Tong
journal: "Economics of Disasters and Climate Change"
year: 2018
volume: "2(3)"
pages: "225-235"
paper_url: "https://link.springer.com/article/10.1007/s41885-018-0028-3"
abstract: "This paper is the first to examine the short term local economic impact of tropical cyclones by estimating the effects on monthly nightlight intensity. More specifically, for Guangdong Province in Southern China, we proxy monthly economic activity with remote sensing derived monthly night time light intensity and combine this with local measures of wind speed derived from a tropical cyclone wind field model. Our regression analysis reveals that there is only a significant (negative) impact in the month of the typhoon strike and nothing thereafter. Understanding that typhoons are inherently a short-term phenomenon has possible implications for studies using more aggregate data."
image: "guangdong.jpg"
featured: true
audio_en: "China.mp3"
topics:
  - Natural Disasters
finding: "Typhoons decrease economic activity in the short term, but recovery is relatively rapid in this context."
media: []
---

Using satellite nightlight data and detailed typhoon track information, we estimate the short-term economic impacts of tropical cyclones in Guangdong Province, China. The methodology allows us to capture economic effects at a fine spatial and temporal resolution.

We find significant negative effects of typhoon exposure on local economic activity, measured by nightlight intensity. However, the effects are concentrated in the immediate aftermath of storms, with relatively rapid recovery in subsequent months.
